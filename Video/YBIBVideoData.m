...
- (void)loadData {
    ...
    if (self.videoAVAsset) {
        //缓存视频
        if (self.cachePath && ![[NSFileManager defaultManager]fileExistsAtPath:self.cachePath]) {
            [self yb_saveToCachePath];
        }else {
...
- (void)yb_saveToCachePath {
    ...
    if (self.videoAVAsset && [self.videoAVAsset isKindOfClass:AVURLAsset.class]) {
           ...
           NSURL *URL = asset.URL;
           if ([URL.scheme isEqualToString:@"file"]) {//本地存在直接播放
             [self.delegate yb_videoData:self readyForAVAsset:self.videoAVAsset];
           } else if ([URL.scheme containsString:@"http"]) {
               if ([[NSFileManager defaultManager]fileExistsAtPath:self.cachePath]) {
                   NSURL *sourcePath = [NSURL fileURLWithPath:self.cachePath];
                    AVAsset *asset = [AVURLAsset URLAssetWithURL:sourcePath options:nil];
                   [self.delegate yb_videoData:self readyForAVAsset:asset];
                  }else {
...
- (void)yb_transformToFuXin {
    if (self.transformToFuXinCallBack) {
        self.transformToFuXinCallBack();
    }
}
#pragma mark - private
...
didCompleteWithError:(nullable NSError *)error {
    if (error) {
        [self.yb_auxiliaryViewHandler() yb_showIncorrectToastWithContainer:self.yb_containerView text:[YBIBCopywriter sharedCopywriter].videoDownloadFailed];
    }
    self.downloading = NO;
}
...
didFinishDownloadingToURL:(NSURL *)location {
    ...
    if (self.cachePath && (![[NSFileManager defaultManager]fileExistsAtPath:self.cachePath])) {
        [[NSFileManager defaultManager] moveItemAtURL:location toURL:[NSURL fileURLWithPath:self.cachePath] error:nil];
        self.downloading = NO;
        [self.delegate yb_videoData:self readyForAVAsset:self.videoAVAsset];
    }else {
